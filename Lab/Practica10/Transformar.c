...
void main() {
  float t1, t2;
  unsigned char mat1[N][N]__attribute__((aligned));
  //unsigned char mat1[N][N];
  int i;
  char tonto;
  unsigned char mat2[N][N]__attribute__((aligned));
  //unsigned char mat2[N][N];
  char tonto2;
  unsigned char mat3[N][N]__attribute__((aligned));
  ...
  leer(mat2,"original.pgm",&info);
   t1 = GetTime();
  for (i=0;i<M;i++)  procesar(&mat1[0][0],&mat2[0][0],&mat3[0][0],N);
  t2 = GetTime();
